...
> = {
  seeking_home: { label: "입양처 찾는 중", short: "입양", emoji: "🏡", color: "#191F28" },
 ...
 * 변천: 5km → 400m → 150m → 3km (2026-07-13 사용자 요청: 등록 지점 기준 반경 3km).
 * 기반 좌표가 이미 ±444m 오프셋이라 보안 충분.
 */
export const CAT_ROAM_RADIUS_M = 3000;
 ...
 *   한 방향으로 쭉 가지 말고 왔다갔다) — 반경 3km에 맞춰 주기 재조정,
 *   시뮬 기준 평균 시속 ~40km, 틱당 ~6m(부드러움), 커버 ~2.4km.
...
export function roamCoord(
  ...
  const nx =
    (0.32 * Math.sin(t / (820 * s1) + f1 * 97) +
      0.34 * Math.sin(t / (130 * s2) + f2 * 59) +
      0.34 * Math.sin(t / (43 * s1) + f1 * 31)) / Math.SQRT2;
  const ny =
    dir *
    (0.32 * Math.cos(t / (870 * s2) + f2 * 89) +
      0.34 * Math.cos(t / (122 * s1) + f1 * 67) +
      0.34 * Math.cos(t / (37 * s2) + f2 * 37)) / Math.SQRT2;
