 ...
 * 변천: 5km → 400m → 150m → 3km → 300m → 2km (2026-08-04 사용자 요청).
 * 등록 지점 기준 2km 안에서 나갔다 돌아오길 반복. 속도 성분(주기 단축)은 유지 —
 * 반경 확대로 평균 이동 속도도 함께 오름 (초속 ~5m → ~25m 체감).
 * 기반 좌표가 이미 ±444m 오프셋이라 보안은 오프셋이 담당.
 */
export const CAT_ROAM_RADIUS_M = 2000;

/**
 * 실제 고양이가 활동반경을 돌아다니는 것처럼, 시간에 따라 불규칙하게 움직이는 표시 좌표.
 * - 기반 좌표(getDisplayCoord — 등록 시점에 이미 ±444m 오프셋된 DB 좌표) 중심으로
 ...
 *   한 방향으로 쭉 가지 말고 왔다갔다). 2026-08-04 사용자 요청으로 반경 300m 축소에
 *   맞춰 주기를 ~5배 단축 — 등록 지점 주변을 활발히 돌다가 아지트로 돌아오길 반복.
 *   시뮬 기준 평균 초속 ~5m(고양이 질주 속도), 저주파(4분) 성분이 "나갔다 돌아옴" 주기.
 *   고양이별 주기 변주(±20%)·회전방향으로 패턴 다양화. √2 정규화로 반경 이내.
 */
/**
...
export function roamCoord(
  ...
  isLoggedIn: boolean,
  tMs: number = Date.now(),
  radiusM: number = CAT_ROAM_RADIUS_M,
): { lat: number; lng: number } {
  const base = getDisplayCoord(cat, isLoggedIn);
  const [f1, f2] = hashToUnitFloats(cat.id + "|roam");
  const t = tMs / 1000;
  // 진폭 합 1.0 → 축별 최대 1. √2로 나눠 대각선에서도 반경 이내 보장.
  const s1 = 0.8 + 0.4 * f1; // 고양이별 주기 변주 (±20%)
  const s2 = 0.8 + 0.4 * f2;
  const dir = f2 > 0.5 ? 1 : -1; // 회전 성향 (시계/반시계)
  const nx =
    (0.32 * Math.sin(t / (240 * s1) + f1 * 97) +
      0.34 * Math.sin(t / (48 * s2) + f2 * 59) +
      0.34 * Math.sin(t / (16 * s1) + f1 * 31)) / Math.SQRT2;
  const ny =
    dir *
    (0.32 * Math.cos(t / (250 * s2) + f2 * 89) +
      0.34 * Math.cos(t / (45 * s1) + f1 * 67) +
      0.34 * Math.cos(t / (14 * s2) + f2 * 37)) / Math.SQRT2;
