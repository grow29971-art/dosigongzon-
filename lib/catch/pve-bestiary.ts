// 도감(배틀 탭) UI용 PVE 로스터 — 냥줍 lib/pve-bestiary.ts 이식 (2026-08-04 P4).
// lib/catch/battle-engine.ts의 PVE_ROSTER와 표시용 필드(key/name/emoji/traits 등)만
// 동기화해서 들고 있는 클라이언트 세이프 버전. 실제 배틀 스탯/스킬은 battle-engine에만
// 있음 — 로스터 종류를 추가/변경하면 이 파일도 같이 업데이트해야 한다.
//
// 냥줍 대비 변경: 조우 기록 저장처가 profiles → catch_profiles(user_id 키,
// pve_seen_keys/pve_defeated_keys TEXT[] — box/supabase_catch_battle_migration.sql).
export interface BestiaryEntry {
  key: string;
  dexNo: number;       // 도감 번호 (No.001부터)
  name: string;
  emoji: string;
  photo: boolean;       // public/pve/{key}.jpg 존재 여부
  category: string;     // 짧은 한 줄 분류명
...
// ── 도감 기록 (서버 전용) ──
// PVE 배틀 결과 처리 라우트 2곳(자동/수동)이 공유. catch_profiles에 pve_seen_keys /
// pve_defeated_keys 컬럼이 없으면(마이그레이션 전) 에러를 그냥 던지고, 호출부가
// try/catch로 감싸 코인/경험치 같은 핵심 보상 로직에 영향을 주지 않게 한다.
// svc 타입은 service-role 클라이언트 제네릭이 복잡해 구조적으로만 느슨하게 받는다.
export async function recordPveEncounter(
  ...
  if (!pveKey) return;
  const { data, error: selErr } = await svc.from("catch_profiles")
    .select("pve_seen_keys,pve_defeated_keys").eq("user_id", userId).maybeSingle();
  ...
  // 행이 없으면 lazy 생성(upsert) — catch_profiles는 가입 트리거 없이 필요 시점에 만든다.
  const { error } = await svc.from("catch_profiles").upsert(
    { user_id: userId, pve_seen_keys: Array.from(seenSet), pve_defeated_keys: Array.from(defeatedSet) },
    { onConflict: "user_id" },
  );
