...
export type CareStateKey = "lonely" | "hungry" | "sulky" | "excited" | "calm";
...
export function careState(fullness: number, mood: number): CareState {
  if (fullness < 35 && mood < 35) {
    return { key: "lonely", emoji: "🥺", label: "그리움", line: "오랜만이에요… 밥 한 끼랑 쓰담 한 번이면 금방 신나요!" };
  }
  if (fullness < 35) {
    return { key: "hungry", emoji: "🍚", label: "밥 시간", line: "슬슬 밥 시간이에요! 간단한 한 끼 어때요?" };
  }
  if (mood < 35) {
    return { key: "sulky", emoji: "😿", label: "시무룩", line: "살짝 시무룩… 쓰다듬어 주면 금방 풀려요" };
  }
  if (fullness >= 70 && mood >= 70) {
    return { key: "excited", emoji: "😸", label: "신남", line: "기분 최고! 골골송이 절로 나와요 🎶" };
  }
  return { key: "calm", emoji: "😺", label: "평온", line: "느긋하게 그루밍하며 쉬는 중이에요" };
}
