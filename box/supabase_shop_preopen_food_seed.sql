-- ⛔ 폐기 (2026-08-12 당일) — 실행하지 말 것.
-- 사장님 결정 변경: 첫 라인업은 "대즐" 브랜드 사료+간식 단독. 기존 시드 상품(캐츠랑·
-- 이나바·오리젠)은 쓰지 않는다. 대즐 SKU·가격 확정 시 v3(supabase_shop_preopen_dazzle_seed.sql)로 대체 예정.
-- 그 사이 선공개는 상품 없이 진행: 접속 팝업(8/12 게시) → 쇼핑탭 오픈 사전알림 옵트인 수집.
-- ══════════════════════════════════════════
