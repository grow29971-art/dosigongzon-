...
--       → 예약 시각은 "그 시각 이후 첫 체크포인트"에 발송된다는 뜻.
--
...
create policy "scheduled_push_select_admin"
  on public.scheduled_pushes for select to authenticated
  using (exists (select 1 from public.admins a where a.user_id = auth.uid()));

-- 생성: 관리자만 + 본인 명의로만
drop policy if exists "scheduled_push_insert_admin" on public.scheduled_pushes;
create policy "scheduled_push_insert_admin"
  on public.scheduled_pushes for insert to authenticated
  with check (
    created_by = auth.uid()
    and exists (select 1 from public.admins a where a.user_id = auth.uid())
  );

-- 수정: 관리자만 (취소 용도). 이미 발송된 건은 건드리지 못하게 pending만 허용.
drop policy if exists "scheduled_push_update_admin" on public.scheduled_pushes;
create policy "scheduled_push_update_admin"
  on public.scheduled_pushes for update to authenticated
  using (
    status = 'pending'
    and exists (select 1 from public.admins a where a.user_id = auth.uid())
  )
