  ...
  return (
    <>
      ...
      <div className="min-h-dvh flex flex-col" style={{
        ...
        {/* ──────── 선택 화면(대기실) — 대시보드 톤: 짙은 배경 + 얇은 테두리 카드 + 컬러 배지 ──────── */}
        {(phase==="select"||phase==="loading") && (
          <div className="flex-1 overflow-y-auto px-4 pb-6" style={{ background: UI.bgGradient }}>
            {/* 누구와 싸울지 */}
            ...
            {myCats.length===0&&<div className="text-center py-16"><p className="text-[14px]" style={{ color: UI.textSub }}>카드가 없어요. 고양이를 등록하면 카드가 생성돼요!</p></div>}
            {selected&&(
        ...
        {phase==="countdown"&&(
          <div className="flex-1 flex flex-col items-center justify-center">
            <div key={countdown} style={{fontSize:countdown===0?64:100,fontWeight:900,color:countdown===0?UI.accent.orange:UI.textMain,textShadow:"0 4px 20px rgba(0,0,0,0.5)",animation:"cdPop 0.55s ease forwards"}}>
            ...
            <div className="flex items-start justify-center gap-1" style={{minHeight:250}}>
              ...
              <div className="flex flex-col items-center shrink-0" style={{minWidth:48, marginTop:72}}>
                <span style={{
                  ...
                  background:"linear-gradient(160deg,#A9CCFF 0%,#5C93F0 45%,var(--color-primary-dark) 100%)",
                ...
                {actionMsg&&<div key={actionMsg} style={{fontSize:9,fontWeight:800,color:"var(--color-primary-dark)",textAlign:"center",maxWidth:60,lineHeight:1.3,marginTop:4,animation:"msgIn 0.3s ease"}}>{actionMsg}</div>}
              </div>
