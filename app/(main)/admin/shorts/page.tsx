  ...
  if (!authChecked) {
    return (
      <div className="min-h-dvh flex items-center justify-center" style={{ background: "#F7F4EE" }}>
        <Loader2 className="animate-spin" size={24} style={{ color: "#4C82BC" }} />
      </div>
    );
  }
  if (!isAdmin) {
    ...
    <div className="min-h-dvh pb-20" style={{ background: "#F7F4EE" }}>
      ...
      <div className="px-4 pt-12 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-[10px] font-extrabold tracking-[0.12em]" style={{ color: "#4C82BC" }}>ADMIN</p>
        ...
        {!editing && (
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleNew} className="flex items-center gap-1 px-3 py-2 rounded-xl text-white active:scale-95" style={{ background: "linear-gradient(135deg, #4C82BC 0%, #3E6FA8 100%)" }}>
              <Plus size={14} />
      ...
      {editing && (
        <div className="mx-4 mb-4 p-4 bg-white rounded-2xl" style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
          ...
          {!draft.youtube_video_id && (
            <Field label="또는 영상 파일 직접 업로드 (mp4 · webm · mov · 50MB 이하)">
              <label className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-surface-alt cursor-pointer active:scale-[0.99]" style={{ background: "#FFF8F2", border: "1px dashed #E8C9A8" }}>
                {uploadingVideo ? <Loader2 size={16} className="animate-spin" style={{ color: "#4C82BC" }} /> : <Film size={16} style={{ color: "#4C82BC" }} />}
                <span className="text-[12.5px] font-bold" style={{ color: "#3E6FA8" }}>
          ...
          <Field label="썸네일 이미지 (옵션 · jpg/png/webp · 5MB 이하)">
            <label className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-surface-alt cursor-pointer active:scale-[0.99]" style={{ background: "#FFF8F2", border: "1px dashed #E8C9A8" }}>
              {uploadingThumb ? <Loader2 size={16} className="animate-spin" style={{ color: "#4C82BC" }} /> : <ImagePlus size={16} style={{ color: "#4C82BC" }} />}
              <span className="text-[12.5px] font-bold" style={{ color: "#3E6FA8" }}>
  ...
  return (
    <div className="bg-white rounded-2xl p-3 flex gap-3" style={{ boxShadow: "0 2px 10px rgba(0,0,0,0.04)" }}>
      <div className="relative w-20 h-28 rounded-xl overflow-hidden shrink-0" style={{ background: "#000" }}>
        ...
        {s.pinned && (
          <span className="absolute top-1 right-1 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: "rgba(76,130,188,0.95)" }} aria-label="고정">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-2 flex-wrap">
          <button onClick={onEdit} className="text-[10.5px] font-bold px-2 py-1 rounded-md" style={{ background: "rgba(76,130,188,0.12)", color: "#3E6FA8" }}>편집</button>
