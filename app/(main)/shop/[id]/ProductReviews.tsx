...
// 2026-09-16 리디자인: 후기는 카드가 아니라 헤어라인 구분선 리스트, 아바타 원형·사진 8px 둥근 사각.

import { useCallback, useEffect, useRef, useState } from "react";
import Image from "next/image";
import { Star, Camera, X, Trash2, User } from "lucide-react";
    ...
    <span className="inline-flex items-center gap-0.5" aria-label={`별점 ${value}점`}>
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          ...
          style={{ color: n <= value ? "var(--color-warning)" : "var(--color-gray-300)" }}
        />
      ))}
    </span>
  );
  ...
  return (
    <div className="mt-8">
        ...
        <div className="flex items-center gap-2">
          ...
          {reviews.length > 0 && (
            <span className="flex items-center gap-1 text-[13px]">
              ...
              <span className="font-semibold text-text-main">{avg}</span>
              <span className="text-text-light">({reviews.length})</span>
            </span>
          )}
        </div>
        {canWrite && !writing && (
          <UIButton variant="weak" size="sm" onClick={() => setWriting(true)}>
            후기 쓰기
          </UIButton>
        )}
      </div>
      ...
      {writing && (
        <div className="mt-3 pt-3" style={{ borderTop: "1px solid var(--color-divider)" }}>
            ...
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} onClick={() => setRating(n)} aria-label={`별점 ${n}점 주기`} className="press">
                <Star
                  ...
                  style={{ color: n <= rating ? "var(--color-warning)" : "var(--color-gray-300)" }}
                />
              </button>
            ))}
          </div>
          ...
          <textarea
            ...
            className="mt-3 w-full p-3 text-[13px] leading-relaxed resize-none outline-none"
            style={inputStyle}
          />
          ...
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            {photos.map((p, i) => (
              <div key={p.preview} className="relative w-16 h-16 overflow-hidden" style={{ border: "1px solid var(--color-border)", borderRadius: "var(--radius-card-sm)" }}>
            ...
            {photos.length < REVIEW_MAX_IMAGES && (
              <button
                ...
                className="w-16 h-16 flex flex-col items-center justify-center gap-0.5 press"
                style={{ border: "1px dashed var(--color-gray-300)", borderRadius: "var(--radius-card-sm)", color: "var(--color-text-light)" }}
                aria-label="사진 첨부"
              >
                <Camera size={18} />
                <span className="text-[11px]">{photos.length}/{REVIEW_MAX_IMAGES}</span>
              </button>
            )}
            <input
              ref={fileInputRef}
          ...
          {error && <p className="mt-2 text-[13px]" style={{ color: "var(--color-error)" }}>{error}</p>}

          <div className="mt-3 flex gap-2">
            <UIButton variant="secondary" className="flex-1" onClick={() => { setWriting(false); setError(null); }} disabled={busy}>
              취소
            </UIButton>
            <UIButton className="flex-[2]" onClick={handleSubmit} disabled={busy}>
              {busy ? "등록 중…" : "후기 등록"}
            </UIButton>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <p className="mt-3 py-6 text-center text-[13px] text-text-light" style={{ borderTop: "1px solid var(--color-divider)" }}>
          아직 후기가 없어요
        </p>
      ) : (
        <div className="mt-3" style={{ borderTop: "1px solid var(--color-divider)" }}>
          {reviews.map((r) => {
            ...
            return (
              <div key={r.id} className="py-4" style={{ borderBottom: "1px solid var(--color-divider)" }}>
                <div className="flex items-center gap-2.5">
                  <div className="relative w-8 h-8 rounded-full overflow-hidden shrink-0 flex items-center justify-center" style={{ background: "var(--color-surface-alt)" }}>
                    {avatar ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-semibold text-text-main truncate">{r.author_name ?? "길집사"}</p>
                    <div className="flex items-center gap-1.5">
                  ...
                  <div className="mt-2.5 flex gap-2">
                    {imgs.map((src, i) => (
                      <button
                        ...
                        className="relative w-20 h-20 overflow-hidden shrink-0 press"
