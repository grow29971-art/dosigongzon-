...
// 섹션은 카드가 아니라 헤어라인으로 구획 (2026-09-16 리디자인).
// 상태 배지는 회색 헤어라인 칩, 취소·환불만 오류색 — ORDER_STATUS_MAP.color는 화면에 쓰지 않는다.
...
export default function OrderDetailPage() {
  ...
  const statusTone = isAborted ? "var(--color-error)" : "var(--color-text-sub)";
    ...
    <div className="pb-24">
      ...
      {loading ? (
        <div className="px-4 mt-4 space-y-3">
          {[0, 1, 2].map((i) => (
            <div key={i} className="animate-pulse" style={{ height: 110, background: "var(--color-surface-alt)", borderRadius: "var(--radius-card-sm)" }} />
          ))}
        </div>
      ) : !order || !status ? (
        ...
        <div className="px-4">
          ...
          <section className="py-4" style={sectionStyle}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[13px] text-text-light">{order.order_number}</span>
              <span
                className="text-[11px] font-medium px-1.5 py-0.5"
                style={{ color: statusTone, border: `1px solid ${isAborted ? statusTone : "var(--color-border)"}`, borderRadius: "var(--radius-square)" }}
              >
                {status.label}
              </span>
            </div>
            ...
            {/* 상태 타임라인 */}
            {isAborted ? (
              <div className="mt-4 py-3 text-center text-[13px] font-medium" style={{ ...noticeStyle, color: statusTone }}>
                    ...
                    <div key={step.key} className={isLast ? "flex flex-col items-center" : "flex-1 flex flex-col"}>
                      <div className={isLast ? "flex items-center" : "flex items-center w-full"}>
                        <div
                          ...
                          style={{ background: reached ? "var(--color-primary)" : "var(--color-gray-200)" }}
                        >
                          {reached && <Check size={13} className="text-white" strokeWidth={3} />}
                        </div>
                        {!isLast && (
                          <div className="flex-1 h-[2px] mx-1" style={{ background: tlIndex > i ? "var(--color-primary)" : "var(--color-gray-200)" }} />
                        )}
                      </div>
                      <span
                        className="text-[11px] font-medium mt-1.5"
                        style={{ color: reached ? "var(--color-text-main)" : "var(--color-text-light)" }}
                      >
                        {step.label}
                      </span>
                    </div>
              ...
              <div className="mt-4 py-3 text-center text-[13px] font-medium text-text-sub" style={noticeStyle}>
                결제를 기다리고 있어요
              </div>
            )}
            ...
            {!isAborted && refundStatus === "requested" && (
              <div className="mt-3 py-2.5 text-center text-[13px] font-medium" style={{ ...noticeStyle, color: "var(--color-warning)" }}>
                환불 요청이 접수됐어요 — 관리자 확인 후 처리돼요
              </div>
            )}
            {!isAborted && refundStatus === "rejected" && (
              <div className="mt-3 py-2.5 text-center text-[13px] font-medium text-text-sub" style={noticeStyle}>
          ...
          {/* 주문 상품 */}
          <section className="py-4" style={sectionStyle}>
            <h2 className="text-[15px] font-bold text-text-main mb-3">주문 상품</h2>
            <div className="space-y-2">
              {order.items.map((item) => (
                <div key={item.id} className="flex items-center justify-between">
                  ...
                  <span className="text-[13px] font-medium text-text-main shrink-0 ml-3 tabular-nums">{formatWon(item.subtotal)}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          {/* 배송지 */}
          <section className="py-4" style={sectionStyle}>
            <h2 className="text-[15px] font-bold text-text-main mb-3 flex items-center gap-1.5">
              <MapPin size={15} className="text-text-light" /> 배송지
            </h2>
            {order.recipient_address ? (
              <>
                <p className="text-[13px] font-semibold text-text-main">{order.recipient_name} · {order.recipient_phone}</p>
            ...
            {order.tracking_number && (
              <div className="mt-2.5">
                <p className="text-[13px] text-text-sub flex items-center gap-1.5">
                  ...
                  <span className="font-semibold text-text-main">{order.tracking_number}</span>
                </p>
                <a
                  href={trackingSearchUrl(order.courier, order.tracking_number)}
                  target="_blank"
                  rel="noreferrer"
                  className="mt-2 press inline-flex items-center gap-1.5 h-8 px-3 text-[13px] font-semibold text-text-main"
          ...
          <section className="py-4" style={sectionStyle}>
            <h2 className="text-[15px] font-bold text-text-main mb-3 flex items-center gap-1.5">
              <CreditCard size={15} className="text-text-light" /> 결제 정보
            </h2>
            <div className="flex items-center justify-between text-[13px] text-text-sub mb-1.5">
              <span>상품금액</span>
              <span className="tabular-nums">{formatWon(order.total_amount)}</span>
            </div>
            <div className="flex items-center justify-between text-[13px] text-text-sub mb-2.5">
              <span>배송비</span>
              <span className="tabular-nums">{order.shipping_fee > 0 ? formatWon(order.shipping_fee) : "무료"}</span>
            </div>
            <div
            ...
            >
              <span>총 결제금액</span>
              <span className="tabular-nums">{formatWon(order.payment_amount)}</span>
            </div>
          ...
          {notice && (
            <p className="mt-4 text-[13px] font-semibold text-center text-text-main">{notice}</p>
          )}
          {error && (
            <p className="mt-4 text-[13px] font-semibold text-center" style={{ color: "var(--color-error)" }}>{error}</p>
          )}
      ...
      {refundOpen && order && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-8" style={{ background: "rgba(0,0,0,0.5)" }}>
          <div className="w-full max-w-[320px] p-5 max-h-[80vh] overflow-y-auto" style={{ background: "var(--color-surface)", borderRadius: "var(--radius-modal)", boxShadow: "var(--shadow-modal)" }}>
            <p className="text-[17px] font-bold text-text-main mb-1.5">
            ...
            {preShipment ? (
              ...
              <>
                  ...
                  {USER_REASONS.map((code) => (
                    <button
                      ...
                      style={{
                        ...
                        border: `1px solid ${reason === code ? "var(--color-primary)" : "var(--color-border)"}`,
                        borderRadius: "var(--radius-input)",
                      }}
                    >
                ...
                <textarea
                  ...
                  maxLength={500}
                  rows={2}
                  className="w-full px-3 py-2.5 text-[13px] outline-none resize-none mb-3"
                  style={inputStyle}
                />
              </>
            )}
            ...
            {modalDecision && (modalDecision.allowed ? (
              <div className="px-3 py-2.5 mb-4 text-[11px] leading-relaxed" style={{ ...noticeStyle, color: "var(--color-sage)" }}>
                <p className="font-semibold">{modalDecision.note}</p>
                <p className="mt-0.5">
              ...
              <p className="px-3 py-2.5 mb-4 text-[11px] font-semibold leading-relaxed" style={{ ...noticeStyle, color: "var(--color-error)" }}>
                {modalDecision.reason}
              </p>
            ))}

            <div className="flex gap-2">
              <UIButton variant="secondary" className="flex-1" onClick={() => setRefundOpen(false)} disabled={submitting}>
                돌아가기
              </UIButton>
              <UIButton
                className="flex-1"
