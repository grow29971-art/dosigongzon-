  ...
  try {
    ...
    await resend.emails.send({
      ...
      html:
        ...
        `<div style="margin:12px 0;padding:12px 14px;background:#FFFFFF;border-radius:8px;white-space:pre-wrap;font-size:14px;line-height:1.6">${escapeHtml(content || "(내용 없음)")}</div>` +
        `<p style="margin:16px 0 0"><a href="${adminUrl}" style="display:inline-block;padding:8px 14px;background:#111111;color:#fff;text-decoration:none;border-radius:6px;font-weight:bold">관리자 페이지 열기</a></p>` +
        `</div>`,
    });
