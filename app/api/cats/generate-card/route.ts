...
features는 rarity와 별개로, 사진에서 "실제로 보이는" 색/무늬/형질만 있는 그대로 적을 것 —
등급을 의식해서 꾸미거나 추측하지 말고, 안 보이면 unknown/빈 배열로 둘 것.`;
...
// 실제 형질 기반 등급(grade_* 컬럼) 계산 — 기존 card_rarity(위 rarity/makeRandomCard)와는
// 독립적인 별도 필드. calculateCatGrade는 순수 함수라 features만 있으면 언제든 재계산 가능.
    ...
    if (apiKey) {
      try {
        ...
        if (parsed.is_real_cat) {
          ...
          // 실제 형질 기반 등급 — 별도 컬럼이라 실패해도 위 카드 생성 결과에는 영향 없음
