...
// 로스터는 분류군별로 묶고, 각 분류군 안에서는 약한 순으로 정렬한다(패턴 감 잡기 쉽게).
// 분류군마다 전투 패턴 성향을 통일해서 "어떤 놈인지" 감이 오게 함:
//  - 곤충·벌레류: 회피 위주 + 잔챙이 다구리(다단히트) + 물어뜯기(약한 상태이상). 낮은 HP.
//  - 포유류: 정면 근접 파이터. 몸통박치기/할퀴기/포효 위주, 회피는 낮지만 방어·공격이 단단함.
//  - 조류: 급강하 기습형. 크리티컬 확률이 높고 pounce/ambush 계열로 급소를 노림.
//  - 파충류: 매복 후 상태이상형. poison/bind로 서서히 옥죄는 스타일(뱀 한 종뿐이라 대표 격).
// (거미·지네·민달팽이는 생물학적으론 곤충이 아니지만, 게임 톤상 "벌레류"로 묶어 표기)
...
export async function POST(req: Request) {
  ...
  // 이 블록만 조용히 실패하고 넘어간다(코인/경험치 등 위 핵심 보상 로직과 완전히 분리 —
  // 같은 update()에 안 쓴 존재 안 하는 컬럼을 섞으면 Promise.all 전체가 깨질 수 있어서 분리함).
  try {
    const pveKey = isBossEncounter ? "boss" : opponent.id.replace(/^pve-/, "");
    ...
    seenSet.add(pveKey);
    if (result.attackerWins) defeatedSet.add(pveKey);
