      ...
      <div className="space-y-6 text-[13px] text-text-sub leading-relaxed">
        <section>
          ...
          <ul className="list-disc pl-4 space-y-1">
            ...
            <li><b>위치정보</b>: 길고양이 등록·지도 조회 시 기기의 GPS 좌표(동의 시). 유저 본인의 위치는 서버에 저장되지 않으며, 고양이 등록 시 입력된 좌표는 자동으로 ±444m 범위의 랜덤 오프셋이 적용되어 저장됩니다. 비로그인 유저에게는 추가로 ±500m의 결정적 퍼징이 적용되어 동(洞) 단위로만 노출됩니다.</li>
            <li><b>업로드 사진</b>: 사진 업로드 시 EXIF 메타데이터(GPS 좌표 포함)는 자동으로 제거된 후 저장됩니다.</li>
            <li><b>Private Circle 데이터</b>: 회원이 직접 생성한 서클 정보(서클명·멤버 목록·초대 상태)는 회원이 서비스를 이용하는 동안 보관되며, 회원 탈퇴 시 즉시 파기됩니다.</li>
        ...
        <section>
          ...
          <ul className="list-disc pl-4 space-y-1">
            ...
            <li><b>전체 채팅</b>: 매일 04:00 KST에 전체 메시지가 일괄 삭제됩니다. (휘발성 일일 단위 채팅)</li>
        ...
        <section>
          ...
          <ul className="list-disc pl-4 space-y-1">
            <li><b>목적</b>: 현재 위치 지도 중심 이동, 근거리 고양이 조회, 고양이 등록 시 위치 핀 표시</li>
            <li><b>수집 방식</b>: 브라우저 Geolocation API — 이용자 동의 시에만</li>
            <li><b>유저 본인 위치</b>: 서버에 저장되지 않습니다. 클라이언트(브라우저) 내에서만 일시적으로 사용됩니다.</li>
            <li><b>등록 좌표 흐림</b>: 회원이 고양이를 등록할 때 입력된 좌표에는 자동으로 약 <b>±444m</b>의 랜덤 오프셋이 적용되어 DB에 저장됩니다. 같은 자리를 두 번 찍어도 매번 다른 값이 저장되어, 등록자 본인조차 정확한 자리를 역추적할 수 없습니다.</li>
            <li><b>비로그인 외부인 추가 흐림</b>: 비회원에게는 위 좌표 위에 추가로 약 <b>±500m</b>의 결정적 퍼징이 적용되어, 합산 최대 약 ±944m의 흔들림이 발생합니다. 사진·이름·동 이름은 비공개이며, 도트와 카운트만 표시됩니다.</li>
            <li><b>공개 범위 선택 (Private Circle)</b>: 등록 시 회원이 직접 공개 범위를 <b>전체 공개·내 서클·나만 보기</b> 중 선택할 수 있습니다. "내 서클" 핀은 회원이 승인한 서클 멤버에게만, "나만 보기" 핀은 본인에게만 노출됩니다(DB 권한 격리로 코드 우회 불가).</li>
            <li><b>동의 철회</b>: 브라우저/OS 설정에서 위치 권한을 거부하시면 서비스에 좌표가 전달되지 않습니다.</li>
          </ul>
        </section>

        <section>
          <h2 className="text-[15px] font-bold text-text-main mb-2">제5조의2-2 (AI 집사 챗봇 데이터)</h2>
          <p className="mb-2">
            AI 집사 챗봇은 Google Gemini API를 통해 작동하며, 다음과 같이 데이터를 처리합니다.
          </p>
          <ul className="list-disc pl-4 space-y-1">
            <li><b>전송 내용</b>: 사용자가 입력한 질문 텍스트만 Google에 전송됩니다. 식별 정보(이메일·닉네임 등)는 함께 전송하지 않습니다.</li>
            <li><b>서버 보관</b>: 도시공존 서버는 <b>대화 내용을 저장하지 않습니다</b>. 응답이 끝난 즉시 메모리에서 소실됩니다.</li>
            <li><b>Google 측 처리</b>: Google Gemini API의 데이터 보존·이용 정책은 Google의 정책을 따릅니다. 자세한 사항은 Google AI 개인정보 정책을 참조하세요.</li>
            <li><b>면책</b>: AI 응답은 참고용이며 의료 진단·법률 자문이 아닙니다. 응급·의료 사안은 반드시 자격 있는 수의사·전문가와 상의해주세요.</li>
          </ul>
        </section>
        ...
        <p className="text-[11px] text-text-light pt-4">시행일: 2026년 4월 21일 · 최종 개정일: 2026년 5월 18일 (위치정보 처리 기준 명확화 ±444m+±500m, Private Circle 데이터 처리 명시, AI 챗봇 데이터 비저장 명시, 사진 EXIF 자동 제거 명시)</p>
      </div>
    </div>
  );
}
