...
export default function DarkCheckPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FFFFFF", color: "#191919", padding: 20 }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>다크모드 진단</h1>
      <p style={{ fontSize: 13, color: "#4B4B4B", marginBottom: 16 }}>
        이 화면을 캡처해서 보내주세요. 각 박스가 무슨 색으로 보이는지가 중요해요.
      </p>

      <div style={{ ...box, background: "#FFFFFF" }}>A: 일반 배경 (원래 흰색)</div>
      <canvas ref={canvasRef} width={340} height={48} style={{ borderRadius: "var(--radius-square-lg)", border: "2px solid #191919", display: "block", marginBottom: 10, maxWidth: "100%" }} />
      ...
      <div style={{ marginTop: 20, padding: 14, borderRadius: "var(--radius-square-lg)", background: "#FFFFFF", fontSize: 13, lineHeight: 1.7, wordBreak: "break-all" }}>
        ...
        <div style={{ marginTop: 8, fontSize: 11, color: "#767676" }}>{info.ua}</div>
      </div>
    </div>
  );
}
