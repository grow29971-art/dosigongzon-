...
export default function DarkCheckPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FFFFFF", color: "#111111", padding: 20, fontFamily: "sans-serif" }}>
      ...
      <canvas ref={canvasRef} width={340} height={48} style={{ borderRadius: "var(--radius-square-lg)", border: "2px solid #111111", display: "block", marginBottom: 10, maxWidth: "100%" }} />
      <div style={{ ...box, background: "none", backgroundImage: "linear-gradient(#FFFFFF, #FFFFFF)" }}>
        C: 그라디언트 배경 (원래 흰색)
      </div>
      <div style={{ ...box, background: "#111111", color: "#FFFFFF", border: "none" }}>D: 테라코타 (원래 주황갈색)</div>
      ...
      <div style={{ marginTop: 20, padding: 14, borderRadius: "var(--radius-square-lg)", background: "#FFFFFF", fontSize: 13, lineHeight: 1.7, wordBreak: "break-all" }}>
        ...
        <div style={{ marginTop: 8, fontSize: 11, color: "#8A8A8A" }}>{info.ua}</div>
      </div>
    </div>
  );
}
