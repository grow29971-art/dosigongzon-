...
export default function HomeReengageCard({ quietCat, regionName, neighborhoodCatCount, latestPost }: Props) {
  ...
  if (!regionName) {
    return (
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-full flex items-center justify-center shrink-0 text-xl" style={{ background: "rgba(76,130,188,0.12)" }}>🗺️</div>
          <div className="flex-1 min-w-0">
            <p className="text-[9.5px] font-extrabold tracking-[0.15em]" style={{ color: "#3E6FA8" }}>우리 동네</p>
  ...
  return (
    <div className="mb-3" style={{ ...cardBox, overflow: "hidden" }}>
      ...
      <Link href="/map" className="flex items-center gap-3 px-3.5 py-3 active:scale-[0.99] transition-transform">
        <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-lg" style={{ background: "rgba(76,130,188,0.12)" }}>🗺️</div>
        <div className="flex-1 min-w-0">
          <p className="text-[9.5px] font-extrabold tracking-[0.15em]" style={{ color: "#3E6FA8" }}>우리 동네 · {regionName}</p>
          <p className="text-[13.5px] font-extrabold text-text-main leading-tight mt-0.5 truncate">
            {neighborhoodCatCount > 0 ? (
              <>돌보는 고양이 <b style={{ color: "#4C82BC" }}>{neighborhoodCatCount}마리</b> 지도에서 보기</>
            ) : (
              <>내 주변 고양이 지도 둘러보기</>
            )}
          </p>
        </div>
        <MapPin size={15} style={{ color: "#4C82BC" }} className="shrink-0" />
      </Link>

      {latestPost && cat && (
        <>
          <div className="h-px mx-3.5" style={{ background: "rgba(76,130,188,0.14)" }} />
          {/* 커뮤니티 최근 글 → 글 상세 */}
