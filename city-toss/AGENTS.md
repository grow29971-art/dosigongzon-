...
- **라우터는 HashRouter**(9/22 실기기: BrowserRouter에서 탭이 간헐 무반응·마이페이지 안 열림 → 해시로 바꾸자 해결). 되돌리지 말 것.
- **외부 링크는 전부 `Device.openURL`**(`src/lib/external-links.ts` 전역 브릿지가 `<a>`·window.open을 가로챔). 평범한 앵커는 WebView가 막아
  9/21 번들 -4가 반려됐다. tel:도 이 경로로 열린다.
- 실기기 오류는 화면 하단 빨간 박스(ErrorToast)·에러 바운더리로 노출된다 — 사장님 스크린샷이 곧 로그다.
